...
namespace ThreeFourteen.Finnhub.Client
{
    public class StockClient
    {
        ...
        public async Task<List<Dividend>> GetDividend(string symbol, DateTime from)
        {
            return await this.GetDividend(symbol, from, DateTime.UtcNow);
        }

        public async Task<List<Dividend>> GetDividend(string symbol, DateTime from, DateTime to)
        {
            if (string.IsNullOrWhiteSpace(symbol)) throw new ArgumentException(nameof(symbol));
